  ...
  it.effect("configures app identity from the environment commit override", () => {
    ...
    return withIdentity(
      Effect.gen(function* () {
        ...
        assert.deepEqual(calls.setName, ["Agent Hub Code (Alpha)"]); // [agent-hub] rebrand
        assert.equal(calls.setAboutPanelOptions[0]?.applicationName, "Agent Hub Code (Alpha)"); // [agent-hub] rebrand
